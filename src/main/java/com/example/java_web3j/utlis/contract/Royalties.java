...
public class Royalties extends Contract {
	...
	public List<SecondarySaleFeesEventResponse> getSecondarySaleFeesEvents(TransactionReceipt transactionReceipt) {
        List<EventValuesWithLog> valueList = extractEventParametersWithLog(SECONDARYSALEFEES_EVENT, transactionReceipt);
    ...
    public Flowable<SecondarySaleFeesEventResponse> secondarySaleFeesEventFlowable(EthFilter filter) {
        return web3j.ethLogFlowable(filter).map(new Function<Log, SecondarySaleFeesEventResponse>() {
            ...
            public SecondarySaleFeesEventResponse apply(Log log) {
                EventValuesWithLog eventValues = extractEventParametersWithLog(SECONDARYSALEFEES_EVENT, log);
                SecondarySaleFeesEventResponse typedResponse = new SecondarySaleFeesEventResponse();
     ...
     * @return
     */
    public MarketItem getIdToMarketItem(BigInteger id) {
        final org.web3j.abi.datatypes.Function function = new org.web3j.abi.datatypes.Function("idToMarketItem",
                Arrays.<Type>asList(new Uint256(id) ),
                Arrays.<TypeReference<?>>asList(new TypeReference<MarketItem>() {}));
        RemoteFunctionCall<MarketItem> listRemoteFunctionCall = executeRemoteCallSingleValueReturn(function, MarketItem.class);
        try {
            MarketItem send = listRemoteFunctionCall.send();
            return send;
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @return
     */
    public List<MarketItem> getFetchMarketItems() {
        final org.web3j.abi.datatypes.Function function = new org.web3j.abi.datatypes.Function("fetchMarketItems",
                Arrays.<Type>asList(),
                Arrays.<TypeReference<?>>asList(new TypeReference<DynamicArray<MarketItem>>() {}));
        RemoteFunctionCall<DynamicArray> listRemoteFunctionCall = executeRemoteCallSingleValueReturn(function, DynamicArray.class);
        try {
            DynamicArray send = listRemoteFunctionCall.send();
            List value = send.getValue();
            return value;
        } catch (Exception e) {
            e.printStackTrace();
        }
